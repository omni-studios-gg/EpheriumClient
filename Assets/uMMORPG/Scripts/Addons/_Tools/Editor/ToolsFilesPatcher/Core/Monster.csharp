    ...
    List<string> validParameters = new List<string>();
...
#if _iMMOTIMEDSKILLFINISH
        if (monsterTimedSkillFinish.StateSkillFinished())
            return "IDLE";
#else
        ...
        string UpdateServer_DEAD()
        {
            ...
            if (EventRespawnTimeElapsed())
            {
                ...
                Revive();
#if _iMMOWORLDBOSSMESSAGE && _SERVER
            if(monsterWorldBossMessage != null && monsterWorldBossMessage.isWorldBoss)
                monsterWorldBossMessage.OnReviveMessage();
#endif
                return "IDLE";
            }
        ...
        public override void OnDeath()
        {
            ...
            // happen immediately if current TimeLogout > end TimeLogout.
            deathTimeEnd = NetworkTime.time + deathTime;
            respawnTimeEnd = deathTimeEnd + respawnTime; // after death TimeLogout ended
        }
        ...
        {
            //Debug.Log("Tools_CanAttack : " + base.Tools_CanAttack(entity) + " | GetAttackAllowance" + GetAttackAllowance(entity));
            return base.Tools_CanAttack(entity)
#if _iMMOPVP
        ...
        protected override void OnInteract()
        {
            ...
            if (player.combatRemastered)
            {
                Debug.Log("(Monster.cs) skill index : "+ player.autoAttack);
                //((PlayerSkills)player.skills).TryUse(player.combatRemastered.SetSkillIndex());
                if(player.combatRemastered)
                    ((PlayerSkills)player.skills).TryUse(player.autoAttack);//player.combatRemastered.TryUseWeaponSkill();
            ...
            else if (health.current == 0 && Utils.ClosestDistance(player, this) <= player.interactionRange && inventory.HasLoot())
            {
