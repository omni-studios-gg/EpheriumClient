...
namespace uMMORPG
{
    ...
    public partial class PlayerEquipment : Equipment
    {
...
#if _SERVER
        [HideInInspector] public bool isDirtyEquipment = false;
#endif
        ...
        public override void OnStartClient()
        {
            ...
            slots.OnChange += OnEquipmentChanged;
#else
...
#pragma warning restore CS0618
            ...
            if (player.playerAddonsConfigurator.tmpl_UMACharacterCreation)
                player.playerAddonsConfigurator.UpdateUma();
#endif
...
#if MIRROR_90_OR_NEWER
        void OnEquipmentChanged(SyncList<ItemSlot>.Operation op, int index, ItemSlot oldSlot)
#else
...
#endif
        {
            ...
            //    items, any TimeLogout we get attacked the equipment durability changes.
...
#if MIRROR_90_OR_NEWER
            ScriptableItem newItem = slots[index].amount > 0 ? slots[index].item.data : null;
#else
...
#if _iMMOITEMLEVELUP
#if MIRROR_90_OR_NEWER
                || oldSlot.item.equipmentLevel != slots[index].item.equipmentLevel
#else
            || oldSlot.item.equipmentLevel != newSlot.item.equipmentLevel
#endif
#endif
                ...
                )
            {
                ...
                RefreshLocation(index);
                if (isLocalPlayer)
                    player.CmdCancelAction(); // Require a cancel action because if you remove the weapon you can't attack
            }
#endif
            onEquipmentChanged.Invoke();
#if _iMMOCOMBATREMASTERED
        if (player.combatRemastered != null)
            player.combatRemastered.AutoRemoveEquipment(index);
#endif
        }
        ...
        public void RefreshLocation(int index)
        {
            ...
            if (info.requiredCategory != "" && info.location != null)
            {
                ...
                if (info.location.childCount > 0)
                {
                    ...
                    if (player.playerAddonsConfigurator.tmpl_UMACharacterCreation)
                    {
                        for (int i = 0; i < info.location.childCount; i++)
                            player.playerAddonsConfigurator.ProcessBones(info.location);
                    }
                    else
#endif
                    info.location.RemoveAllChildren();
...
#if _iMMOITEMLEVELUP
                if (itemData.enableLevelUp && slot.item.equipmentLevel > 0 && slot.item.equipmentLevel <= itemData.LevelUpParameters.Length && itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.particleEffectModifier != null)
                {
                    GameObject go = Instantiate(itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.particleEffectModifier, info.location, false);
                    go.name = itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.particleEffectModifier.name;
                }

                if (itemData.enableLevelUp && slot.item.equipmentLevel > 0 && slot.item.equipmentLevel <= itemData.LevelUpParameters.Length && itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.ModelPrefab != null)
                {
                    GameObject go = Instantiate(itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.ModelPrefab, info.location, false);
                    go.name = itemData.LevelUpParameters[slot.item.equipmentLevel - 1].equipmentLevelUpModifier.ModelPrefab.name;
                ...
                {
                    // clear previous one in any case (when overwriting or clearing)
                    if (info.location != null && info.location.childCount > 0) Destroy(info.location.GetChild(0).gameObject);
...
#if _iMMOITEMLEVELUP
            inventory.CheckResetEquipmentLevelUp(inventory.slots[slotIndices[0]].item);
#endif
            }
        }
        ...
        void OnDragAndDrop_EquipmentSlot_InventorySlot(int[] slotIndices)
        {
            ...
            bool allowRemoveItem = false;
            bool allowRemoveItemCursed = true;
            int extraSize = 0;

            // Equipable bag test.
            if (slots[slotIndices[0]].amount > 0)
            {
                EquipmentItem item = (EquipmentItem)slots[slotIndices[0]].item.data;
                extraSize = item.extraInventorySize;
            }
            if (player.Tools_inventorySlotCount(player) <= (player.playerAddonsConfigurator.inventorySize - extraSize) || extraSize == 0)
            {
                allowRemoveItem = true;
            }
            /*
#if _iMMOCURSEDEQUIPMENT
            if (slots[slotIndices[0]].amount > 0 && slots[slotIndices[0]].item.data is EquipmentItem itemData && itemData.CanUnequip(player, slotIndices[1], slotIndices[0]))
                allowRemoveItemCursed = true;
#endif
            */
            if (allowRemoveItem && allowRemoveItemCursed)
            {

                CmdSwapInventoryEquip(slotIndices[1], slotIndices[0]); // reversed
#if _iMMOITEMLEVELUP
                inventory.CheckResetEquipmentLevelUp(inventory.slots[slotIndices[1]].item);
#endif
            }
#else
        ...
        public void SwapInventoryEquip(int inventoryIndex, int equipmentIndex)
        {
            ...
            if (inventory.InventoryOperationsAllowed() && 0 <= inventoryIndex && inventoryIndex < inventory.slots.Count && 0 <= equipmentIndex && equipmentIndex < slots.Count)
            {
                // item slot has to be empty (unequip) or equipabable
                ItemSlot slotInventory = inventory.slots[inventoryIndex]; // inventory item
                ItemSlot slotEquipment = slots[equipmentIndex]; // equipment item
                EquipmentItem itemDataEquipment = null;
                if (slotEquipment.amount > 0)
                {
                    itemDataEquipment = (EquipmentItem)slotEquipment.item.data;
                }
                EquipmentItem itemDataInventory = null;
                if (slotInventory.amount > 0)
                {
                    itemDataInventory = (EquipmentItem)slotInventory.item.data;
                }

                //Debug.Log("slotInventory amount : " + slotInventory.amount + " | slotEquipment amount : " +slotEquipment.amount);
                // on à un item équipé et on veut l'enlever et le placé dans un emplacement vide
                if (slotEquipment.amount > 0 && slotInventory.amount == 0 && itemDataEquipment != null && itemDataEquipment.CanUnequip(player, inventoryIndex, equipmentIndex)) // <-- deplacé un equipment vers l'inventaire sur un emplacement vide
                {
                    //Debug.Log("on appelle bien canUneqip de equipment ver inventaire");
                    // swap them
                    ItemSlot temp = slots[equipmentIndex];
                    inventory.slots[inventoryIndex] = temp;
                    slots[equipmentIndex] = slotInventory;

                    inventory.isDirtyInventory = true;
                    isDirtyEquipment = true;
#if _iMMOITEMLEVELUP
                inventory.CheckResetEquipmentLevelUp(slotInventory.item);
#endif
                }
                else if (slotEquipment.amount > 0 && slotInventory.amount > 0 && slotInventory.item.data is EquipmentItem itemDataIn && itemDataIn.CanUnequip(player, inventoryIndex, equipmentIndex)) // <-- deplacer de l'equipment vers l'inventaire sur un slot non vide
                {
                    //Debug.Log("on appelle bien canUneqip");
                    // swap them
                    ItemSlot temp = slots[equipmentIndex];
                    inventory.slots[inventoryIndex] = temp;
                    slots[equipmentIndex] = slotInventory;
                ...
                else if (slotEquipment.amount == 0 && slotInventory.amount > 0 && itemDataInventory != null && itemDataInventory.CanUnequip(player, inventoryIndex, equipmentIndex)) // <-- on va deplacé un item de l'inventaire vers l'equipment vide
                {
                    //Debug.Log("on appelle bien canUneqip de inventaire vers equipment");
                    // swap them
                    ItemSlot temp = slots[equipmentIndex];
                    inventory.slots[inventoryIndex] = temp;
                    slots[equipmentIndex] = slotInventory;
                ...
                {
                    //Debug.Log("ok je sais pas, probablement pas quelque chose d'utile");
                }


                //if (slot.amount == 0 ||  //! slot.amount == 0, cette ligne n'est pas bonne car il faudrait auussi vérifier si le slot est vide et si l'item n'est pas un cursed equipment
                /*if (slotInventory.amount > 0 && slotInventory.item.data is EquipmentItem itemData &&
    #if _iMMOCURSEDEQUIPMENT
                    itemData.CanUnequip(player, inventoryIndex, equipmentIndex)
    #else
...
#if _SERVER
            // validate
            if ((player.state == "IDLE" || player.state == "MOVING" || player.state == "CASTING") && 0 <= index && index < slots.Count && slots[index].amount > 0)
            {
                // check inventory for free slot and pass it to swapinventoryequip()
                ItemSlot item = slots[index];

#if _iMMOCURSEDEQUIPMENT
            if (inventory.SlotsFree() >= item.amount && ((EquipmentItem)item.item.data).CanUnequipClick(player, (EquipmentItem)item.item.data))
#else
        ...
        {
#if _SERVER
            // reduce weapon durability by one each TimeLogout we attacked someone
