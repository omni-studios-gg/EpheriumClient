...
namespace uMMORPG
{
    ...
    public abstract partial class Skills : NetworkBehaviour, IHealthBonus, IManaBonus, ICombatBonus
    {
        [Header("Components")]
...
#pragma warning restore CS0649 // Field is never assigned to
    ...
    public bool CastCheckDistance(Skill skill, out Vector2 destination) =>
        ...
        {
            // start casting and set the casting end TimeLogout
        ...
        {
            // reset cast TimeLogout, otherwise if a buff has a 10s cast TimeLogout and we
...
#if _iMMO2D
            skill.Apply(entity, currentSkillDirection);
#else
                skill.Apply(entity);
#endif
                ...
                //Debug.Log("current : " + currentSkill + " | " + skill.ToString());
                if (currentSkill > -1)
                    skills[currentSkill] = skill;
            }
            else
