...
namespace uMMORPG
{
    ...
    public partial class PlayerCharacterControllerMovement : Movement
    {
...
#pragma warning restore CS0109 // member does not hide accessible member
        ...
        [Header("[-=-[ Enable AutoRun ]-=-")]
        public bool enableAutoRun;
        public KeyCode autorunButton;
        private bool running = false;


        [Header("[-=-[ Enable Double Jump ]-=-]")]
...
#pragma warning restore
        ...
        void Start()
        {
            ...
            if (isLocalPlayer) // TODO use OnStartLocalPlayer instead?
            {
                ...
                InitializeFreeLook();
                freelookWorldAngle = freeLookParent.eulerAngles.y;
            }
        ...
        if (useMobileController)
        {
            running = MobileControls.GetAutorun();
        }
#endif

            if (enableAutoRun && running)
            {
        ...
        if (useMobileController)
        {
            Vector3 worldDir = freeLookParent.TransformDirection(new Vector3(inputDir.x, 0f, inputDir.y));
            return worldDir;
        }
        else
#endif
        ...
        MoveState UpdateAIRBORNE(Vector2 inputDir, Vector3 desiredDir)
        {
            ...
            else if (enableDoubleJump && EventJumpRequested())
            {
                if (doublejump < doublejumplimit)
                {
                    // start the jump movement into Y dir, go to jumping
                    // note: no endurance>0 check because it feels odd if we can't jump
                    moveDir.y = jumpSpeed;
...
#if _iMMOMOBILECONTROLS
            if ((Input.GetMouseButtonDown(0) || MobileControls.GetTouchDown) && !Utils.IsCursorOverUserInterface())
            {
                MobileControls.pointerDown = true;
            }
#endif
...
#if _iMMOMOBILECONTROLS
                if (MobileControls.GetJump())
                {
                    jumpKeyPressed = MobileControls.GetJump();
                ...
                if (enableAutoRun && Input.GetKeyDown(autorunButton))
                {
                    running = !running;
                }
            }
        ...
        /* TRUGORD */
        float autoFaceTowardsSpeed = 800f; // 800 est une vitesse correcte pour ce tourné vers la destination
        float freelookWorldAngle;
        ...
        void UpdateCamera()
        {
            ...
            if (health.current > 0)
            {
                ...
                if (Input.GetMouseButton(mouseFreeLookButton) && !Utils.IsCursorOverUserInterface() && UIDragAndDropable.currentlyDragged == null)
                {
            ...
            {
#endif
                if (Input.GetMouseButton(mouseFreeLookButton) && !Input.GetMouseButton(notFreeLookButton) && (dragDistance >= freeLookDragThreshold || Cursor.lockState == CursorLockMode.Locked) && !Utils.IsCursorOverUserInterface() &&
                                                                                                                                                            UIDragAndDropable.currentlyDragged == null && UIWindow.currentlyDragged == null && distance > 0)
                {
            ...
            if (useMobileController)
            {
                // Cette condtion à la place
                if ((Input.touchCount == 1 && MobileControls.joy_tIdx == -1) || (Input.touchCount == 2 && MobileControls.joy_tIdx != -1))
                {

                    // we are rotating with a mouse
                    //rotatingWithMouse = true;

                    int cam_tID = (MobileControls.joy_tIdx == 0) ? 1 : 0;
                    Touch camTouch = Input.GetTouch(cam_tID);

                    if (!MobileControls.IsTouchOverUserInterface(camTouch))
                    {
                        // Down Event
                        if (!camDrag)
                        {
                            camDrag = (camTouch.phase == TouchPhase.Began && !MobileControls.IsTouchOverUserInterface(camTouch));
                        }

                        // Drag Event
                        //if (camDrag && camTouch.phase == TouchPhase.Moved)
                        if (camDrag)// Rotation de la caméra
                        {
                            Vector2 delta = MobileControls.Get1FingerPositionChange(camTouch);
                            rotation.y += delta.y * YSensitivity;
                            rotation.x -= delta.x * XSensitivity;
                            rotation.x = Mathf.Clamp(rotation.x, MinimumX, MaximumX);
                            //camera.transform.rotation = Quaternion.Euler(rotation.x, rotation.y, 0);

                            freelookWorldAngle += delta.y * YSensitivity;

                            camera.transform.Rotate(new Vector3(-(delta.x * XSensitivity), 0, 0));

                            //transform.rotation = Quaternion.Slerp(camera.transform.rotation, Quaternion.Euler(rotation.x, rotation.y, 0), rotationSpeed * .6f);
                ...
                // we do this in LateUpdate so it's consistent with camera's update order
                if (IsMoving()) // Rotation du joueur
                {
                    // character auto rotate mode
                    // velocity to euler rotation
                    Vector2 dir = new Vector2(velocity.x, velocity.z);
                    dir.Normalize();
                    float yAngle = Mathf.Atan2(dir.x, dir.y);
                    yAngle = Mathf.Rad2Deg * yAngle;

                    Vector3 eul = transform.eulerAngles;
                    eul.y = Mathf.MoveTowardsAngle(eul.y, yAngle, autoFaceTowardsSpeed * Time.deltaTime);
                    transform.eulerAngles = eul;
                }

                freeLookParent.eulerAngles = Vector3.up * freelookWorldAngle;
                //freeLookParent.eulerAngles = Vector3.up * yAngle;
            }
#endif
            }
...
#if _iMMOMOBILECONTROLS
        if (useMobileController) {
            camera.transform.position = Vector3.Lerp(target, (target - (camera.transform.rotation * Vector3.forward * finalDistance)), 0.5f);
        }
        else
#endif
