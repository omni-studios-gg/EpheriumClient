    ...
    public class Inventory : ItemContainer
    {
        ...
        public int Count(Item item)
        {
            ...
            foreach (ItemSlot slot in slots)
                ...
                if (slot.amount > 0 && item.data.GetType() == slot.item.data.GetType() && item.data == slot.item.data)
                    amount += slot.amount;
            return amount;
        }
        ...
        public bool Remove(Item item, int amount)
        {
            for (int i = 0; i < slots.Count; ++i)
            {
                ...
                // note: .Equals because name AND dynamic variables matter (petLevel etc.)
                //Debug.Log(slot.item.name + " || " + item.name);
                //if (slot.amount > 0 && slot.item.Equals(item))
                if (slot.amount > 0 && item.data.GetType() == slot.item.data.GetType() && item.data == slot.item.data)
                {
        ...
        public bool CanAdd(Item item, int amount)
        {
            ...
            for (int i = 0; i < slots.Count; ++i)
            {
                ...
                else if (item.data.GetType() == slots[i].item.data.GetType() && item.data == slots[i].item.data)
        ...
        public bool Add(Item item, int amount)
        {
            ...
            if (CanAdd(item, amount))
            {
                ...
                for (int i = 0; i < slots.Count; ++i)
                {
                    ...
                    if (slots[i].amount > 0 && item.data.GetType() == slots[i].item.data.GetType() && item.data == slots[i].item.data)
                    {
