...
namespace uMMORPG
{
    ...
    Utils.GetNearestTransform(startPositions, from);
        ...
        public override void Awake()
        {
            ...
            //    select/create/delete each TimeLogout!
...
#if _iMMOGAMEVERSION
        if(message.text == "outdated version")
        {
            popupUpdateGame.Show(downloadUri);
        } 
        else
#endif
            // show a popup
        ...
        CharactersAvailableMsg MakeCharactersAvailableMessage(string account)
        {
            ...
            //Debug.Log("<color=red>juste la selection des perso ?</color>");
        ...
        void LoadPreview(GameObject prefab, Transform location, int selectionIndex, CharactersAvailableMsg.CharacterPreview character)
        {
            ...
            //Debug.Log("<color=red>juste � la selection des perso ? pour la preview</color>");
        ...
        if (player.playerAddonsConfigurator.tmpl_UMACharacterCreation)
        {
            ...
            player.playerAddonsConfigurator.umaDna = character.umaDna;
            ...
            for (int i = 0; i < character.equipment.Length; ++i)
            {
                ...
                if (slot.amount > 0)
                {
                    ...
                    // Cmd_OnEquipmentChanged won't be called unless spawned, we
        ...
        void OnClientCharactersAvailable(CharactersAvailableMsg message)
        {
            ...
            //Debug.Log("nbr char ="  + charactersAvailableMsg.characters.Length);
...
#if _iMMOUMACHARACTERS
    private Player CreateCharacter(GameObject classPrefab, string characterName, string account, bool gameMaster, string dna)
#else
#if _iMMO2D
    private Player CreateCharacter(GameObject classPrefab, string characterName, string account)
#else
        private Player CreateCharacter(GameObject classPrefab, string characterName, string account, bool gameMaster)
#endif
#endif
        {
            // create new character based on the prefab.
            // -> we also assign default items and equipment for new characters
            // -> skills are handled in Database.CharacterLoad every TimeLogout. if we
...
#if _iMMOUMACHARACTERS
        if(player.playerAddonsConfigurator)
            player.playerAddonsConfigurator.umaDna = dna;
#endif
...
#if _iMMOSTAMINA
            player.stamina.current = player.stamina.max; // after equipment in case of boni
#endif
#if !_iMMO2D
                        ...
                        if (Database.singleton.CharactersForAccount(account).Count < characterLimit)
                        {
                            ...
                            if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                            {
                                    ...
                                    conn == NetworkServer.localConnection)
                                {
                                    ...
                                    // create new character based on the prefab.
#if _iMMOUMACHARACTERS
                                Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account, message.gameMaster, message.dna);
#else
#if _iMMO2D
                                Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account);
                                    ...
                                    // save the player
#if _MYSQL
                                    //Database.singleton.CharacterSave(player, false, true, true);
                                    Database.singleton.CharacterCreate(player, false, true, true);
#else
                                Database.singleton.CharacterSave(player, false, true);
#endif
        ...
        void OnServerCharacterSelect(NetworkConnectionToClient conn, CharacterSelectMsg message)
        {
            ...
            if (lobby.ContainsKey(conn))
            {
                ...
                if (0 <= message.index && message.index < characters.Count)
                {
                    ...
                    // remove from lobby
#if _iMMOLOBBY
                if(networkManagerMMONetworkZones == null) // if networkmanager dont have networkManagerMMONetworkZones (axemplev classicscene)
                    lobby.Remove(conn);
#else
                    lobby.Remove(conn);
#endif
        ...
        IEnumerator<WaitForSeconds> DoServerDisconnect(NetworkConnectionToClient conn, float delay)
        {
            ...
            {
#if _MYSQL
                Database.singleton.CharacterSave(conn.identity.GetComponent<Player>(), false, true, true);
#else

            Database.singleton.CharacterSave(conn.identity.GetComponent<Player>(), false, true);
#endif
                //Database.singleton.CharacterSave(conn.identity.GetComponent<Player>(), false, true, false);
                //Database.singleton.CharacterSaveDisconnect(conn.identity.GetComponent<Player>(), false, true, true);
                Debug.Log("saved:" + conn.identity.name);
            }

#if _iMMOLOBBY && _MYSQL
        if (networkManagerMMONetworkZones != null && lobby.ContainsKey(conn)) // Add update account if player logout or transfert to other server
        {
            //Debug.Log("Account Name : "+ lobby[conn]);
            Database.singleton.LogoutAccountUpdate(lobby[conn], false);
        }
#endif
            // addon system hooks
