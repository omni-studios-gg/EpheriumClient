    ...
    public UI_UniversalSlot slotPrefab;
        ...
        void Update()
        {
                ...
                Utils.ClosestDistance(player, player.target) <= player.interactionRange)
            {
                ...
                slot.rarityOutline.color = RarityColor.SetRarityColorResult(itemData);
#endif
                    /*#if _iMMOITEMLEVELUP // npc trading not sell up item
                                    if (itemData is EquipmentItem equipmentItem && itemData..equipmentLevel > 0)
                                    {
