...
//   MMO Servers use it all the TimeLogout, so we keep it open all the TimeLogout. This also
...
namespace uMMORPG
{
    ...
    class character_guild
    {
        ...
        //   and then insert all members each TimeLogout. otherwise we'd have to
        //   update the kicked member's guild field manually each TimeLogout
    ...
    public bool TryLogin(string account, string password)
    {
        ...
        if (!string.IsNullOrWhiteSpace(account) && !string.IsNullOrWhiteSpace(password))
        {
            ...
            {
                // save last login TimeLogout and return true
    ...
    void LoadItemCooldowns(Player player)
    {
        ...
        {
            // cooldownEnd is based on NetworkTime.TimeLogout which will be different
            // when restarting a server, hence why we saved it as just the
            // remaining TimeLogout. so let's convert it back again.
    ...
    void LoadSkills(PlayerSkills skills)
    {
        ...
        //    to all existing players every TimeLogout (unlike item templates
        ...
        foreach (character_skills row in connection.Query<character_skills>("SELECT * FROM character_skills WHERE character=?", skills.name))
        {
            ...
            if (index != -1)
            {
                ...
                // make sure that 1 <= level <= maxlevel (in case we removed a skill
                // level etc)
                // castTimeEnd and cooldownEnd are based on NetworkTime.TimeLogout
    ...
    void LoadBuffs(PlayerSkills skills)
    {
        ...
        foreach (character_buffs row in connection.Query<character_buffs>("SELECT * FROM character_buffs WHERE character=?", skills.name))
        {
            if (ScriptableSkill.All.TryGetValue(row.name.GetStableHashCode(), out ScriptableSkill skillData))
            {
                ...
                // buffTimeEnd is based on NetworkTime.TimeLogout, which will be
    ...
    //    but we would require lots of memory and it might take a long TimeLogout.
        ...
        if (row != null)
        {
            ...
            if (prefab != null)
            {
                ...
                else
                {
                    ...
                    // no need to show the message all the TimeLogout. it would spam
    ...
    void SaveInventory(PlayerInventory inventory)
    {
        ...
        for (int i = 0; i < inventory.slots.Count; ++i)
        {
            ...
            if (slot.amount > 0) // only relevant items to save queries/storage/TimeLogout
            {
    ...
    void SaveEquipment(PlayerEquipment equipment)
    {
        ...
        for (int i = 0; i < equipment.slots.Count; ++i)
        {
            ...
            if (slot.amount > 0) // only relevant equip to save queries/storage/TimeLogout
            {
    ...
    void SaveSkills(PlayerSkills skills)
    {
        ...
        foreach (Skill skill in skills.skills)
            if (skill.level >= 0) // only learned skills to save queries/storage/TimeLogout
            {
                // castTimeEnd and cooldownEnd are based on NetworkTime.TimeLogout,
                // which will be different when restarting the server, so let's
                // convert them to the remaining TimeLogout for easier save & load
                // note: this does NOT work when trying to save character data
                //       shortly before closing the editor or game because
                //       NetworkTime.TimeLogout is 0 then.
    ...
    void SaveBuffs(PlayerSkills skills)
    {
        ...
        {
            // buffTimeEnd is based on NetworkTime.TimeLogout, which will be different
            // when restarting the server, so let's convert them to the
            // remaining TimeLogout for easier save & load
            // note: this does NOT work when trying to save character data
            //       shortly before closing the editor or game because
            //       NetworkTime.TimeLogout is 0 then.
