...
namespace uMMORPG
{
    ...
    public partial class PlayerParty : NetworkBehaviour
    {
        [Header("Components")]
        ...
        {
            // clear proximity cache instead of allocating a new one each TimeLogout
            proximity.Clear();
...
#if _iMMOSETTINGS
                && !other.playerAddonsConfigurator.isBlockingParty
#endif
                    )
                {
                    // send an invite
