...
namespace uMMORPG
{
    ...
    public partial class PlayerLooting : NetworkBehaviour
    {
        [Header("Components")]
        ...
        public void CmdTakeGold()
        {
            ...
            if ((player.state == "IDLE" || player.state == "MOVING" || player.state == "CASTING") &&
                player.target != null &&
#if _iMMOTOOLS
            player.Tools_SelectionHandling_DeadMonster((Entity)player.target) &&
#else
...
#if _iMMOTOOLS
            player.Tools_SelectionHandling_DeadMonster(monster) &&
#else
                player.target.health.current == 0 &&
#endif
                Utils.ClosestDistance(player, player.target) <= player.interactionRange)
