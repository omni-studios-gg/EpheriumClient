...
namespace uMMORPG
{
    ...
    public partial class UIPlayerTrading : MonoBehaviour
    {
        ...
        {
    #if UNITY_EDITOR
            string prefabName = "SlotUniversalPlayerTrading"; // Nom du prefab par défaut
    ...
    [CustomEditor(typeof(UIPlayerTrading))]
    public partial class UIPlayerTradingEditor : Editor
    {
        private string prefabName = "SlotUniversalPlayerTrading"; // Nom du prefab par défaut
    
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
    
            UIPlayerTrading uiPlayerTrading = (UIPlayerTrading)target;
    
            // Vérifier si le champ slotPrefab est assigné
            bool isAssigned = uiPlayerTrading.slotPrefab != null;
            ...
            if (GUILayout.Button(buttonText))
            {
                if (isAssigned)
                {
                    Debug.Log("Slot Prefab (Universal) is already assigned: " + uiPlayerTrading.slotPrefab.name);
                }
                else
                {
                    AssignPrefab(uiPlayerTrading);
                }
            }
        ...
        private void AssignPrefab(UIPlayerTrading uiPlayerTrading)
        {
            ...
            if (guids.Length > 0)
            {
                ...
                if (selectedPrefab != null)
                {
                    ...
                    if (universalSlot != null)
                    {
                        ...
                        uiPlayerTrading.slotPrefab = universalSlot;
                        EditorUtility.SetDirty(uiPlayerTrading);
