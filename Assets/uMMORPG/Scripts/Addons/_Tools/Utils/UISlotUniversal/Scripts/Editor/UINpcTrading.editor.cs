...
namespace uMMORPG
{
    ...
    public partial class UINpcTrading : MonoBehaviour
    {
        ...
        {
    #if UNITY_EDITOR
            string prefabName = "SlotUniversalNpcTrading"; // Nom du prefab par défaut
    ...
    {
        private string prefabName = "SlotUniversalNpcTrading"; // Nom du prefab par défaut
    
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
    
            UINpcTrading uiNpcTrading = (UINpcTrading)target;
    
            // Vérifier si le champ slotPrefab est assigné
            bool isAssigned = uiNpcTrading.slotPrefab != null;
            ...
            if (GUILayout.Button(buttonText))
            {
                if (isAssigned)
                {
                    Debug.Log("Slot Prefab (Universal) is already assigned: " + uiNpcTrading.slotPrefab.name);
                }
                else
                {
                    AssignPrefab(uiNpcTrading);
                }
            }
        ...
        private void AssignPrefab(UINpcTrading uiNpcTrading)
        {
            ...
            if (guids.Length > 0)
            {
                ...
                if (selectedPrefab != null)
                {
                    ...
                    if (universalSlot != null)
                    {
                        ...
                        uiNpcTrading.slotPrefab = universalSlot;
                        EditorUtility.SetDirty(uiNpcTrading);
