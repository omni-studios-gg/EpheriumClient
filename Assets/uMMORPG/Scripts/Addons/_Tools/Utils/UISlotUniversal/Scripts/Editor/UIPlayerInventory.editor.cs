...
namespace uMMORPG
{
    ...
    public partial class UIInventory : MonoBehaviour
    {
        ...
        {
    #if UNITY_EDITOR
            string prefabName = "SlotUniversalInventory"; // Nom du prefab par défaut
    ...
    public partial class UIInventoryEditor : Editor
    {
        private string prefabName = "SlotUniversalInventory"; // Nom du prefab par défaut
    
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
    
            UIInventory uiInventory = (UIInventory)target;
    
            // Vérifier si le champ slotPrefab est assigné
            bool isAssigned = uiInventory.slotPrefab != null;
            ...
            if (GUILayout.Button(buttonText))
            {
                if (isAssigned)
                {
                    Debug.Log("Slot Prefab (Universal) is already assigned: " + uiInventory.slotPrefab.name);
                }
                else
                {
                    AssignPrefab(uiInventory);
                }
            }
        ...
        private void AssignPrefab(UIInventory uiInventory)
        {
            ...
            if (guids.Length > 0)
            {
                ...
                if (selectedPrefab != null)
                {
                    ...
                    if (universalSlot != null)
                    {
                        ...
                        uiInventory.slotPrefab = universalSlot;
                        EditorUtility.SetDirty(uiInventory);
